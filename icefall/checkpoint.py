...
def average_checkpoints(
    ...
    avg = torch.load(filenames[0], map_location=device, weights_only=True)["model"]
    ...
    for i in range(1, n):
        state_dict = torch.load(filenames[i], map_location=device, weights_only=True)["model"]
...
def average_checkpoints_with_averaged_model(
    ...
    Args:
      ...
      device:
        Move checkpoints to this device before averaging.
    """
    state_dict_start = torch.load(filename_start, map_location=device, weights_only=True)
    state_dict_end = torch.load(filename_end, map_location=device, weights_only=True)
