...
class Zipformer2EncoderLayerS(nn.Module):
    """
    Args:
        embed_dim: the number of expected features in the input (required).
        nhead: the number of heads in the multiheadattention models (required).
    ...
    def __init__(
        ...
        super(Zipformer2EncoderLayerS, self).__init__()
        self.embed_dim = embed_dim
        ...
        self.feed_forward1 = FeedforwardModule(
            embed_dim, (feedforward_dim * 2) // 3, dropout
        )

        self.feed_forward2 = FeedforwardModule(embed_dim, feedforward_dim, dropout)

        self.feed_forward3 = FeedforwardModule(
            embed_dim, (feedforward_dim * 4) // 3, dropout
        )

        self.nonlin_attention = NonlinAttention(
            embed_dim, hidden_channels=3 * embed_dim // 8
        )
...
class Zipformer2S(EncoderInterface):
    """
    Args:
    ...
    def __init__(
        ...
        super(Zipformer2S, self).__init__()
        ...
        num_encoders = len(downsampling_factor)
        for i in range(num_encoders):
            encoder_layer = Zipformer2EncoderLayerS(
                embed_dim=encoder_dim[i],
                pos_dim=pos_dim,
                num_heads=num_heads[i],
                query_head_dim=query_head_dim[i],
