...
def compute_validation_loss(
    ...
    # 写入文件
    write_logs_to_gz(params.manifest_dir / "zipformer_s_55_loss_logs.jsonl.gz", valid_logs)
...
def run(rank, world_size, args):
    ...
    if params.teacher_model_id != "zipformer_s_55":
        print("错误：teacher_model_id 设置", file=sys.stderr)
        sys.exit(1)  # 非零退出码表示异常退出
    
    valid_cuts = aishell.train_multilayer_distill_cuts_s()
