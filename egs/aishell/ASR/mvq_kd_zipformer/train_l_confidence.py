...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        default="512,768,1536,2048,1536,768",
    ...
    parser.add_argument(
        ...
        default="192,256,512,768,512,256",
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="1,7",
        help="Distillation layer index of teacher model",
    )

    parser.add_argument(
        "--teacher-model-id",
        type=str,
        default="zipformer_s_55",
        help="could be one of:  zipformer_s_55",
    )
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        ...
        middle_layer_outputs = losses[-3]
        
    return simple_loss, pruned_loss, middle_layer_outputs
...
def compute_validation_loss(
    ...
    from dynamic_confidence import write_logs_to_gz

    # 在验证循环中的调用示例
    valid_logs = []
    for batch_idx, batch in enumerate(valid_dl):
        # print(f"batch {batch_idx} : ", batch)
        # print(f"batch {batch_idx} total frames : ", torch.sum(batch["supervisions"]["num_frames"]))
        feature = batch["inputs"]
        simple_loss, pruned_loss, middle_layer_outputs = compute_loss(
            params=params,
            model=model,
        ...
        print(f"batch_idx : {batch_idx}  batch shape : {feature.shape}")
        assert simple_loss.requires_grad is False

        # 收集日志
        valid_logs.append({
            "batch_idx": batch_idx,
            "feature_shape": feature.shape,  # 原始Tensor形状
    ...
    # 写入文件
    write_logs_to_gz(params.manifest_dir / "zipformer_l_56_loss_logs.jsonl.gz", valid_logs)
...
def run(rank, world_size, args):
    ...
    train_cuts = aishell.valid_cuts()
    # valid_cuts = aishell.valid_cuts()
    # valid_cuts = aishell.train_multilayer_distill_cuts()

    if params.teacher_model_id != "zipformer_l_56":
        print("错误：teacher_model_id 设置", file=sys.stderr)
        sys.exit(1)  # 非零退出码表示异常退出
    
    valid_cuts = aishell.train_multilayer_distill_cuts_l()
    ...
    valid_dl = aishell.train_dataloaders(
        valid_cuts, sampler_state_dict=sampler_state_dict
    )
