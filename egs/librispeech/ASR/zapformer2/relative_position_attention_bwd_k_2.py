...
@triton.jit
def relative_position_attention_bwd_k_kernel(
    # fmt: off
        ...
        BLOCK_N: tl.constexpr,  # block size in q
        BLOCK_C: tl.constexpr,  # block size for seq_q
    ...
    num_pid_m = tl.cdiv(seq_k, BLOCK_M)
    ...
    q_base = q_ptr + batch * stride_qb + head * stride_qh + offs_n[:, None] * stride_qc
    k_base = k_ptr + batch * stride_kb + head * stride_kh
    pos_base = pos_ptr + head * stride_ph + offs_n[:, None] * stride_pc
    scores_grad_base = (
        scores_grad_ptr + batch * stride_sb + head * stride_sh + offs_m * stride_sk
    )
    ...
    for c in range(0, channels, BLOCK_C):
        ...
        # (BLOCK_M, BLOCK_C), or (K, I), or (1, BLOCK_C)
        scores_grad_mask = (offs_m < seq_k) & (c_idx[None, :] < seq_q)

        # (BLOCK_N, BLOCK_C), or (J, I)
        q_mask = offs_n_mask & (c_idx[None, :] < seq_q)

        # (BLOCK_M, BLOCK_C), or (K, I), or (1, BLOCK_C)
        rel_idx = c_idx[None, :] - offs_m + max_seq_len - 1

        # (BLOCK_M, BLOCK_N, BLOCK_C), or (K, J, I), or (BLOCK_N, BLOCK_C)
        pos_mask = (rel_idx >= 0) & (rel_idx < 2 * max_seq_len - 1) & offs_n_mask

        scores_grad_ptrs = scores_grad_base + c_idx[None, :] * stride_sq
        q_ptrs = q_base + c_idx[None, :] * stride_qs
        ...
        # scores_grad_chunk   (1, BLOCK_C), or (K, I)
        # q_chunk             (BLOCK_N, BLOCK_C), or (J, I)
        # pos_chunk           (BLOCK_N, BLOCK_C), or (J, I)
        qp = q_chunk * pos_chunk

        acc += tl.sum(scores_grad_chunk * qp, axis=1)

    k_ptrs = k_base + offs_m * stride_ks + offs_n * stride_kc
    k_mask = (offs_m < seq_k) & (offs_n < channels)
    tl.store(k_ptrs, acc, mask=k_mask)


def relative_position_attention_bwd_k(scores_grad, q, pos):
    ...
    assert scores_grad.ndim == q.ndim == 4, (scores_grad.shape, q.shape)
    ...
    assert scores_grad.device == q.device == pos.device, (
        scores_grad.device,
        q.device,
        pos.device,
    )

    k = torch.empty(b, h, seq_k, c, device=q.device)

    grid = lambda META: (
        triton.cdiv(seq_k, META["BLOCK_M"]) * triton.cdiv(c, META["BLOCK_N"]),
        b * h,
    )

    # fmt:off
    relative_position_attention_bwd_k_kernel[grid](
            q, k, pos, scores_grad,
...
def benchmark(b, h, seq_q, seq_k, c, provider):
    ...
    if provider == "triton":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: relative_position_attention_bwd_k(scores_grad, q, pos),
            quantiles=quantiles,
        )
...
def test_correctness():
    ...
    print("k.grad", k.grad.shape, k.grad.sum())

    scores_grad = scores0.grad.clone()
    k_grad = relative_position_attention_bwd_k(scores_grad, q_copy, pos_copy)

    print(k_grad.shape, k_grad.sum())
