...
from relative_position_attention_module_optimized import RelativePositionAttentionFunction
import torch
    ...
    def forward(
        ...
        """
            ...
            Args:
             ...
             pos_emb: (1, 2*seq_len-1, head_dim) or (batch_size, 2*seq_len-1, head_dim)
...
dropout:
    ...
    def __init__(
        ...
        self.encoder_pos = CompactRelPositionalEncoding(
            head_dim, dropout_rate=0.0, length_factor=1.0
        )
        self.name = None
...
class RelPositionMultiheadAttentionWeights(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            pos_emb: Positional embedding tensor, of shape (1, 2*seq_len - 1, head_dim)
        ...
        q = q.permute(1, 2, 0, 3)  # (batch, head, time1, query_head_dim)
        k = k.permute(1, 2, 0, 3)  # (batch, head, time2, query_head_dim)

        if self.training:
            k = with_loss(k,
                          self.qk_max_product(q.reshape(batch_size * num_heads, seq_len, query_head_dim),
                                              k.reshape(batch_size * num_heads, seq_len, query_head_dim),
                                              aux_loss_scale / num_heads),
                          None)


        attn_scores = RelativePositionAttentionFunction.apply(q.contiguous(), k.contiguous(), pos_emb.repeat(num_heads, 1, 1))


        assert attn_scores.shape == (batch_size, num_heads, seq_len, seq_len)
        attn_scores = attn_scores.permute(1, 0, 2, 3)
    ...
    def streaming_forward(
        ...
        # HERE.. not finished streaming code.
        if torch.jit.is_tracing():
