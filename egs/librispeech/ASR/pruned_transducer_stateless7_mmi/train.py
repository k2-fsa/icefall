...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Scale for aux loss (ctc loss or mmi loss)",
    )
...
def compute_loss(
    ...
    # Compute auxiliary loss (mmi loss or ctc loss)
    ...
    if batch_idx_train < warm_step:
        ...
        assert aux_loss.requires_grad == is_training
        info["ctc_loss"] = aux_loss.detach().cpu().item()
        info["mmi_loss"] = 0
    else:
        ...
        aux_loss = loss_fn(dense_fsa_vec=dense_fsa_vec, texts=texts)
        assert aux_loss.requires_grad == is_training
        info["ctc_loss"] = 0
        info["mmi_loss"] = aux_loss.detach().cpu().item()

    loss += params.aux_loss_scale * aux_loss
