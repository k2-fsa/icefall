...
class ConvRNNT(EncoderInterface):
    ...
    Args:
        ...
        num_GlobalCNNEncoder_layers (int): number of Global CNN Encoder layers
    ...
    def __init__(
        ...
        num_features: int,
        d_model: int = 512,
        LocalCNNEncoder_kernel_size: int = 5,
        num_GlobalCNNEncoder_layers: int = 6,
        dropout: float = 0.1,
        ...
        self.encoder_layers = num_GlobalCNNEncoder_layers
        self.d_model = d_model
        self.causal = causal

        self.LocalCNN_encoder = LocalCNNEncoder(
            kernel_size=LocalCNNEncoder_kernel_size,
            causal=causal,
        )
        
        # aux_layers from 1/3
        self.GlobalCNN_encoder = GlobalCNNEncoder(
            encoder_layer=GlobalCNNEncoderLayer,
            channels=d_model,
            dropout=dropout,
            num_layers=num_GlobalCNNEncoder_layers,
            aux_layers=list(
                range(
                    num_GlobalCNNEncoder_layers // 3,
                    num_GlobalCNNEncoder_layers - 1,
                    aux_layer_period,
                )
            ),
            causal=causal,
    ...
    def forward(
        ...
        src_x = self.LocalCNN_encoder(
            x,
            src_key_padding_mask=src_key_padding_mask,
        )  # (T, N, C)

        x = self.GlobalCNN_encoder(
            src_x,
            warmup=warmup,
        )  # (T, N, C)
    ...
    def get_init_state(
        ...
        Returns:
          ...
          (num_GlobalCNNEncoder_layers, encoder_dim).
...
class GlobalCNNEncoder(nn.Module):
    ...
    Examples::
        ...
        >>> GlobalCNN_encoder = GlobalCNNEncoder(encoder_layer, num_layers=6)
        >>> src = torch.rand(10, 32, 512)
        >>> out = GlobalCNN_encoder(src)
    """
...
class LocalCNNEncoder(nn.Module):
    ...
    def forward(
        ...
        if src_key_padding_mask is not None:
            x = x.masked_fill_(src_key_padding_mask.unsqueeze(1).expand_as(x), 0.0)
...
class GlobalCNNEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        # 1D Depthwise Conv
        if src_key_padding_mask is not None:
            out.masked_fill_(src_key_padding_mask.unsqueeze(1).expand_as(out), 0.0)
        if self.causal and self.lorder > 0:
...
class ConvolutionModule(nn.Module):
    """ConvolutionModule in ConvRNNT model.
