...
from conformer_ctc import ConformerCTC, compute_distillation_loss
...
def get_parser():
    ...
    parser.add_argument(
        ...
        help="How to aggregate multi-layer distillation losses: "
             "'layer_avg' computes loss for each layer and averages them, "
             "'output_avg' averages the layer outputs first then computes a single loss.",
    )
...
def compute_loss(
    ...
    if 'clean' in batch and 'noisy' in batch and params.enable_self_distillation:
        ...
        # Move to device
        clean_feature = clean_feature.to(device)
        noisy_feature = noisy_feature.to(device)
    ...
    with torch.set_grad_enabled(is_training):
        ...
        model_output = model(feature, supervisions)
        
        # Extract outputs from ConformerCTC
        nnet_output = model_output['ctc_output']         # (N, T, C)
        distill_hidden = model_output['distill_hidden']  # (N, T, d_model)
        encoder_memory = model_output['encoder_out']     # (N, T, d_model)
        memory_mask = None  # ConformerCTC doesn't return mask, we'll create it from lengths
        
        # Create memory mask from output lengths
        output_lens = model_output['encoder_out_lens']
        max_len = nnet_output.size(1)
        memory_mask = torch.arange(max_len, device=device)[None, :] >= output_lens[:, None]
        
        # Self-distillation computation
        distillation_loss = torch.tensor(0.0, device=device)
        if use_self_distillation and params.enable_self_distillation:
            # Use EMA teacher model if available, otherwise use clean samples
            if ema_teacher is not None:
                # Get teacher model outputs (teacher is always in eval mode)
                teacher_model = ema_teacher.get_teacher_model()
                with torch.no_grad():
                    # Use noisy samples for teacher model (student samples)
                    teacher_model_output = teacher_model(feature, supervisions)
            elif clean_feature is not None:
                # Fallback to clean samples as teacher
                teacher_model_output = model(clean_feature, clean_supervisions)
            else:
                # No teacher available, skip distillation
                teacher_model_output = None
            
            if teacher_model_output is not None:
                ...
                if params.knowledge == "encoder-output":
                    ...
                    if 'distill_outputs' in model_output and 'distill_outputs' in teacher_model_output:
                        teacher_outputs = teacher_model_output['distill_outputs']
                        student_outputs = model_output['distill_outputs']
                        
                        # Import the multi-layer distillation function
                        from conformer_ctc import compute_multi_layer_distillation_loss
                        
                        distillation_loss = compute_multi_layer_distillation_loss(
                            teacher_knowledge=teacher_outputs,
                            student_knowledge=student_outputs,
                        ...
                        # Fallback to single layer distillation for backward compatibility
                        from conformer_ctc import compute_distillation_loss
                        distillation_loss = compute_distillation_loss(
                            teacher_knowledge=teacher_model_output['distill_hidden'],
                            student_knowledge=model_output['distill_hidden'],
                            knowledge_lens=output_lens,
                            loss_type=params.distill_loss_type,
                            knowledge_type="encoder-output",
                        )
                    ...
                    if 'attention_maps' in model_output and 'attention_maps' in teacher_model_output:
                        teacher_attention = teacher_model_output['attention_maps']
                        student_attention = model_output['attention_maps']
                        
                        # Import the multi-layer distillation function
                        from conformer_ctc import compute_multi_layer_distillation_loss
                        
                        distillation_loss = compute_multi_layer_distillation_loss(
                            teacher_knowledge=teacher_attention,
                            student_knowledge=student_attention,
                            knowledge_lens=output_lens,
                            layer_indices=distill_layers,
    ...
    # Add self-distillation loss
    if use_self_distillation and distillation_loss.item() > 0:
        total_loss = (1.0 - params.alpha) * total_loss + params.alpha * distillation_loss
...
def run(rank, world_size, args):
    ...
    # Determine encoder layers for self-distillation
    num_encoder_layers = 12  # Default value, can be made configurable
    
    # Parse distillation layers
    distill_layers = []
    if params.enable_self_distillation and params.distill_layers:
        try:
            distill_layers = [int(x.strip()) for x in params.distill_layers.split(',')]
            logging.info(f"Self-distillation ENABLED with layers: {distill_layers}")
            logging.info(f"Knowledge type: {params.knowledge}")
            logging.info(f"Loss type: {params.distill_loss_type}")
            logging.info(f"Aggregation: {params.distill_aggregation}")
            if params.knowledge == "attention-map":
                logging.info(f"Temperature: {params.distill_temperature}")
        except:
            distill_layers = [int(params.distill_layers)]
            logging.info(f"Self-distillation ENABLED with single layer: {distill_layers[0]}")
    else:
        logging.info("Self-distillation DISABLED")
    ...
    model = ConformerCTC(
        ...
        knowledge_type=params.knowledge,
    )
