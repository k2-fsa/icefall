...
from model import DenoisingAsrModel
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Loss scale on the speech part of the loss",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Multiply by base-dim to determine dimension of time embedding."
    )
...
def get_speech_embed(params: AttributeDict) -> nn.Module:
    # speech_embed converts the input of shape (N, T, num_features)
    ...
    # sampling rate.
    speech_embed = Conv2dSubsampling(
...
def get_encoder_model(params: AttributeDict) -> nn.Module:
    encoder = Zapformer(
        ...
        time_embed_dim=lookup(params, "time_embed_dim"),
...
def get_model(params: AttributeDict) -> nn.Module:
    ...
    #speech_embed = get_speech_embed(params)
    encoder = get_encoder_model(params)


    model = DenoisingAsrModel(
        #speech_embed=speech_embed,
        encoder=encoder,
        encoder_dim=lookup(params, "embed_dim"), # see embed-multiple
        text_embed_dim=lookup(params, "text_embed_dim"),
        vocab_size=params.vocab_size,
        time_embed_dim=lookup(params, "time_embed_dim") # see time-embed-multiple
    )
    return model
...
def compute_loss(
    ...
    x_lens = supervisions["num_frames"].to(device)

    batch_idx_train = params.batch_idx_train

    texts = batch["supervisions"]["text"]
    y = sp.encode(texts, out_type=int)  # list of lists.
    y_lens = [ len(sent) for sent in y ]
    max_y_len = max(y_lens)
    y = [ sent + [ 0 ] * (max_y_len - len(sent)) for sent in y ]
    y = torch.tensor(y).to(device)
    y_lens = torch.tensor(y_lens).to(device)

    info = MetricsTracker()
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        nframes = (x_lens // params.subsampling_factor).sum().item()
        info["frames"] = nframes

    with torch.set_grad_enabled(is_training):
        speech_loss, text_loss = model(x, x_lens, y, y_lens)
        # (speech_loss - 2 * nframes).relu() is to prevent it from completely ignoring the speech loss.
        loss = params.speech_loss_scale * speech_loss +  (speech_loss - (2.0 * nframes)).relu() +  text_loss
    ...
    info["speech_loss"] = speech_loss.detach().cpu().item()
