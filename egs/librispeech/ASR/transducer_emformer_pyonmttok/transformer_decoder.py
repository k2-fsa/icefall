...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        Args:
          ...
          nhead:
            ...
            Must satisfy embedding_dim // nhead == 0.
          dim_feedforward:
        ...
        self.decoder_num_class = (
            vocab_size  # bpe model already has sos/eos symbol
        )

        self.decoder_embed = nn.Embedding(
            num_embeddings=self.decoder_num_class, embedding_dim=embedding_dim
        )
        self.decoder_pos = PositionalEncoding(embedding_dim, dropout)
        ...
        self.decoder_output_layer = torch.nn.Linear(
            embedding_dim, self.decoder_num_class
        )
    ...
    def forward(
        ...
        tgt_mask = generate_square_subsequent_mask(
            ys_in_pad.shape[-1] #, self.left_context
        ).to(device)

        tgt_key_padding_mask = decoder_padding_mask(
            ys_in_pad, ignore_id=self.blank_id
        )
...
class TransformerDecoderLayer(nn.Module):
    ...
    Examples::
        >>> decoder_layer = nn.TransformerDecoderLayer(embedding_dim=512, nhead=8)
    ...
    def __init__(
        ...
        self.self_attn = nn.MultiheadAttention(embedding_dim, nhead, dropout=0.0)
...
class PositionalEncoding(nn.Module):
    ...
    Note::
      ...
      1 / (10000^(2i/embedding_dim)) = exp(-log(10000^(2i/embedding_dim)))
                               = exp(-1* 2i / embedding_dim * log(100000))
                               = exp(2i * -(log(10000) / embedding_dim))
    """
    ...
    def __init__(self, embedding_dim: int, dropout: float = 0.1) -> None:
        ...
        self.xscale = math.sqrt(self.embedding_dim)
        self.dropout = nn.Dropout(p=dropout)
        # not doing: self.pe = None because of errors thrown by torchscript
        self.pe = torch.zeros(1, 0, self.embedding_dim, dtype=torch.float32)

    def extend_pe(self, x: torch.Tensor) -> None:
        """Extend the time t in the positional encoding if required.

        The shape of `self.pe` is (1, T1, embedding_dim). The shape of the input x
        is (N, T, embedding_dim). If T > T1, then we change the shape of self.pe
        to (N, T, embedding_dim). Otherwise, nothing is done.
        ...
        if self.pe is not None:
            if self.pe.size(1) >= x.size(1):
                ...
                return
        pe = torch.zeros(x.size(1), self.embedding_dim, dtype=torch.float32)
        ...
        pe = pe.unsqueeze(0)
        # Now pe is of shape (1, T, embedding_dim), where T is x.size(1)
...
def generate_square_subsequent_mask(
    ...
    # if left_context < sz:
    #     mask_left_context = (
    #         (torch.triu(torch.ones(left_context, left_context)) == 1)
    #         .transpose(0, 1)
    #         .float()
    #     )
    #     mask[-left_context:, :left_context] -= mask_left_context
    mask = (
        mask.float()
