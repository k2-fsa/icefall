...
from asr_datamodule_ubiqus_lm import UbiqusLMDataModule
...
from model_lm_adaptation import (
    Transducer,
    TransducerStep2,
    TransducerStep2Hybrid,
)
...
def get_transformer_decoder_model(params: AttributeDict) -> nn.Module:
...
def get_transducer_step2_model(
    base_model: nn.Module, transformer_decoder: nn.Module
) -> nn.Module:
    model = TransducerStep2Hybrid(
        decoder=base_model.decoder,
        joiner=base_model.joiner,
...
def load_checkpoint_if_available(
    ...
    filename = params.exp_dir / f"checkpoint-2640000.pt"
...
def save_checkpoint(
    ...
    if rank != 0:
        return
    filename = params.exp_dir / f"step2_hybrid/epoch-{params.cur_epoch}.pt"
    ...
    if params.best_train_epoch == params.cur_epoch:
        best_train_filename = params.exp_dir / "step2_hybrid/best-train-loss.pt"
        copyfile(src=filename, dst=best_train_filename)

    if params.best_valid_epoch == params.cur_epoch:
        best_valid_filename = params.exp_dir / "step2_hybrid/best-valid-loss.pt"
...
def compute_loss(
    ...
    with warnings.catch_warnings():
        ...
        info["frames"] = sum([len(s) for s in y_list])

    # Note: We use reduction=sum while computing the loss.
    info["loss"] = loss.detach().cpu().item()
    info["loss_step_2"] = loss.detach().cpu().item()
    info["ilm"] = ilm.detach().cpu().item()
    info["kl"] = kl.detach().cpu().item()
...
def train_one_epoch(
    ...
    def maybe_log_param_relative_changes():
        ...
        elif params.batch_idx_train % 20 == 1:
            optimizer.step()
            optimizer.zero_grad()
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        if (
            ...
            save_checkpoint_with_global_batch_idx(
                out_dir=params.exp_dir / "step2_hybrid/",
...
def run(rank, world_size, args):
    ...
    base_model = get_transducer_model(params)

    checkpoints = load_checkpoint_if_available(params=params, model=base_model)

    transformer_decoder = get_transformer_decoder_model(params)
    model = get_transducer_step2_model(base_model, transformer_decoder)
    ...
    ubiqus_lm = UbiqusLMDataModule(args)

    sampler_state_dict = None

    # sampler_state_dict = None

    train_dl = ubiqus_lm.train_dataloaders()

    valid_dl = ubiqus_lm.valid_dataloaders()
...
def main():
    parser = get_parser()
    UbiqusLMDataModule.add_arguments(parser)
