...
from model_lm_adaptation import Transducer, TransducerStep2
...
def get_transducer_step2_model(base_model: nn.Module) -> nn.Module:
    model = TransducerStep2(
...
def load_checkpoint_if_available(
    ...
    filename = params.exp_dir / f"checkpoint-2432000.pt"
...
def save_checkpoint(
    ...
    if rank != 0:
        return
    filename = params.exp_dir / f"step2/epoch-{params.cur_epoch}.pt"
    ...
    if params.best_train_epoch == params.cur_epoch:
        best_train_filename = params.exp_dir / "step2/best-train-loss.pt"
        copyfile(src=filename, dst=best_train_filename)

    if params.best_valid_epoch == params.cur_epoch:
        best_valid_filename = params.exp_dir / "step2/best-valid-loss.pt"
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        if (
            ...
            base_model.decoder = model.transducer.decoder
            base_model.joiner = model.transducer.joiner
            save_model = base_model
            save_checkpoint_with_global_batch_idx(
                out_dir=params.exp_dir / "step2/",
...
def run(rank, world_size, args):
    ...
    model = get_transducer_step2_model(base_model)
