...
class EmformerEncoderLayer(nn.Module):
    ...
    def _apply_lconv_forward(
        self,
        right_context_utterance: torch.Tensor,
        R: int,
    ) -> torch.Tensor:
        """Apply lconv in training and validation mode."""
        utterance = right_context_utterance[R:]
        right_context = right_context_utterance[:R]
        utterance, right_context = self.lconv(utterance, right_context)
        right_context_utterance = torch.cat([right_context, utterance])
        return right_context_utterance

    def _apply_lconv_infer(
        self,
        right_context_utterance: torch.Tensor,
        R: int,
        lconv_cache: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Apply lconv module on utterance in inference mode."""
        utterance = right_context_utterance[R:]
        right_context = right_context_utterance[:R]
        utterance, right_context, lconv_cache = self.lconv.infer(
            utterance, right_context, lconv_cache
        )
        right_context_utterance = torch.cat([right_context, utterance])
    ...
    def infer(
        ...
        Args:
           ...
           lconv_cache (torch.Tensor, optional):
             Cache tensor of left context for causal convolution.
           padding_mask (torch.Tensor):
...
class Emformer(EncoderInterface):
    ...
    def forward(
        ...
        ctc_output = self.ctc_output(output)

        return output, output_lengths, ctc_output
    ...
    def infer(
        ...
        return output, output_lengths, output_states, ctc_output
