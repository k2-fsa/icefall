...
class TriStateScheduler(LRScheduler):
    def __init__(
        self,
        optimizer: Optimizer,
        init_lr: float,
        ...
        super(TriStateScheduler, self).__init__(optimizer, verbose)
        self._step = 0
        self._rate = init_lr
        self.phase = phase
    ...
    def get_lr(self):
        if self._step < self.phase[0] * self.total_steps:
            ...
            self._step >= self.phase[0] * self.total_steps
            and self._step < (1 - self.phase[-1]) * self.total_steps
        ):
            lr = self.warmup_lr
        else:
            lr = max(
                self.warmup_lr
                - (self._step - (1 - self.phase[2]) * self.total_steps)
                * self.decay_factor,
                self.end_lr,
            )
        self._step += 1
...
def _test_tristate():
    m = torch.nn.Linear(100, 100)
    optim = torch.optim.Adam(m.parameters(), lr=1e-5, betas=(0.9, 0.98), eps=1e-08)
