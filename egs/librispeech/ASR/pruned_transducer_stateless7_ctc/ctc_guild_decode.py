...
def decode_one_batch(
    ...
    non_blank_mask = (ctc_output[:, :, 0] < math.log(0.99)) * (~padding_mask)
    T_range = torch.arange(encoder_out.shape[1], device=device)

    # print(
    #     f"remain {non_blank_mask.sum() / (~padding_mask).sum()} "
    #     "after filtering in this batch"
    # )

    frames_list, lens_list = [], []
    for i in range(encoder_out.shape[0]):
        # mask = non_blank_mask[i]
        # keep_blank_indexes = mask.nonzero(as_tuple=True)[0][1:] - 1
        # mask[keep_blank_indexes] = True
        # indexes = torch.masked_select(T_range, mask)
