...
class Zipformer(EncoderInterface):
    ...
    def forward(
        ...
        """
        seed = x.size(0); torch.manual_seed(seed); torch.cuda.manual_seed(seed); torch.cuda.manual_seed_all(seed)
        ...
        # print(f"before downsample {x=}"); breakpoint()
...
class AttentionDownsample(torch.nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, downsample: int):
        ...
        self.query = nn.Parameter(torch.zeros(in_channels) * (in_channels**-0.5))
        # self.query = nn.Parameter(torch.linspace(-1, 1, in_channels) * (in_channels**-0.5))
...
class SimpleUpsample(torch.nn.Module):
    ...
    def __init__(self, num_channels: int, upsample: int):
        ...
        seed = 3; torch.manual_seed(seed); torch.cuda.manual_seed(seed); torch.cuda.manual_seed_all(seed)
