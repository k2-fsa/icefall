...
def get_parser():
    ...
    parser.add_argument(
        "--exp-dir",
        type=str,
        default="lightweight_ctc/exp_phone",
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Out channels of the first layer. Scales network width.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="How many convolutional blocks to use. Scales network depth.",
    )
...
def compute_loss(
    ...
    # at entry, feature is (N, T, F)
    feature = feature.unsqueeze(1)
    assert feature.ndim == 4    # (N, 1, T, F)
    ...
    # nnet_output is (N, T, num_labels + 1)
...
def run(rank, world_size, args):
    ...
    if rank == 0:
        ...
        try:
            ...
            model_stats = torchinfo.summary(
                model, input_size=(1, 1, 100, params.feature_dim))
            logging.info(str(model_stats))
        except ImportError:
            logging.info('torchinfo not installed. Cannot print model summary.')
    ...
    if "lang_bpe" in str(params.lang_dir):
        # For phone lexicon the filter removes too much data.
