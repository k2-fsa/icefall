...
class Wav2Vec2Config(FairseqDataclass):
    ...
    wavlm: bool = field(default=False, metadata={"help": "wavlm"})
    ...
    def forward(
        ...
        **kwargs,
        ...
        if viewmaker is not None:
            ...
            pac_output = features_newview.clone()
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, padding_mask=None, layer=None, tgt_layer=None, layer_wise_detach=None):
        x, layer_results, dropped_layer = self.extract_features(x, padding_mask=padding_mask,tgt_layer=tgt_layer, layer_wise_detach=layer_wise_detach)
...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.self_attn_ins_layer_norm = LayerInstanceNorm(self.embedding_dim, elementwise_affine=False)
...
class TransformerSentenceEncoderLayerPos(nn.Module):
    ...
    def __init__(
        ...
        self.activation_fn = utils.get_activation_fn(activation_fn)
        self.self_attn = MultiheadAttentionRelativePos(
            self.embedding_dim,
            num_attention_heads,
            dropout=attention_dropout,
            self_attention=True,
...
class LayerInstanceNorm(nn.Module):
    ...
    def forward(self, input):
        ...
        mean = mean.reshape(1, mean.size()[0], 1)
        std = std.reshape(1, std.size()[0], 1)
