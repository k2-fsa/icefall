...
To use the generated file with `zapformer/decode.py`,
you can do:
...
To use the generated file with `zapformer/decode.py` and `zapformer/streaming_decode.py`, you can do:
...
- non-streaming model:
https://huggingface.co/Zengwei/icefall-asr-librispeech-zapformer

- streaming model:
https://huggingface.co/Zengwei/icefall-asr-librispeech-streaming-zapformer

with the following commands:

    sudo apt-get install git-lfs
    git lfs install
    git clone https://huggingface.co/Zengwei/icefall-asr-librispeech-zapformer
    git clone https://huggingface.co/Zengwei/icefall-asr-librispeech-streaming-zapformer
...
class StreamingEncoderModel(nn.Module):
    """A wrapper for encoder and encoder_embed (streaming)"""
    ...
    def forward(
        ...
        Args:
            ...
            states: a list of Tensors.
                states[:-2] are the encoder caches (9 tensors per layer).
        ...
        cached_embed = states[-2]
        x, x_lens, new_cached_embed = self.encoder_embed.streaming_forward(
            x=features,
