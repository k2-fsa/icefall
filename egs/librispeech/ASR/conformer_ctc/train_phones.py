...
def get_parser():
    ...
    parser.add_argument(
        ...
        (disabled by default; not supported yet)
        """,
    )
...
def run(rank, world_size, args):
    ...
    model.to(device)
    if world_size > 1:
        # Note: find_unused_parameters=True is needed because we're
        # not training decoder at all by default (for now).
...
# Workaround to see logs with some pytorch versions...
