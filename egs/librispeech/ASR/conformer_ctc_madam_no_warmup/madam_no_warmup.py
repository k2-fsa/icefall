...
class Madam(Optimizer):
    ...
    def __init__(
        ...
        if not (
            isinstance(grad_norm_buffer_size, int) and grad_norm_buffer_size > 1
        ):
            raise ValueError(
                "Invalid grad_norm_buffer_size value: {}".format(
                    grad_norm_buffer_size
                )
            )
        if not limit_grad_factor > 1.0:
            raise ValueError(
                "Invalid limit_grad_factor: {}".format(limit_grad_factor)
            )
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            target_rms_values = (
                []
            )  # relates to weight decay.  Target root-mean-square
            # values of the elements of each parameter
            # we are optimizing
            prev_norm_stats = (
                []
            )  # contains Tensor with 2 elements each, the sum
            ...
            for p in group["params"]:
                if p.grad is not None:
                    ...
                    if len(state) == 0:
                        ...
                        state["grad_norm_buf"] = torch.zeros(
                            grad_norm_buffer_size, 2, device=p.device
                        )
                    ...
                    if limit_grad_factor != float("inf"):
                        ...
                        cur_grad_norm = (
                            p.grad ** 2
                        ).sum()  # actually squared nom
                        prev_mean_norm = grad_norm_buf.sum(
                            0
                        )  # prev_mean_norm is a Tensor [ tot_norm_squared, count ]
                        grad_norm_buf[cur_step % grad_norm_buffer_size][
                            0
                        ] = cur_grad_norm
                        grad_norm_buf[cur_step % grad_norm_buffer_size][
                            1
            ...
            if limit_grad_factor != float("inf"):
                self._apply_grad_norm_clipping(
                    group["params"],
                    prev_norm_stats,
                    cur_grad_norms,
                    grads,
    ...
    def _apply_grad_norm_clipping(
        ...
        all_prev_norm_stats, all_cur_grad_norms = _to_device(
            "cpu", torch.stack(prev_norm_stats), torch.stack(cur_grad_norms)
        )
        ...
        # a finite grad)
        all_prev_grad_norms = (
            all_prev_norm_stats[:, 0] / all_prev_norm_stats[:, 1]
        )
        ...
        if cur_norm - cur_norm != 0:  # cur_norm is infinite or NaN
            ...
            logging.warning(
                f"Infinite gradient-norm detected (cur/prev: {cur_norm}/{prev_norm}): will "
            ...
            if cur_norm > prev_norm * limit_grad_factor2:
                ...
                cur_norm_f, prev_norm_f, grad_factor_f = (
                    "%.2g" % cur_norm,
                    "%.2g" % prev_norm,
                    "%.2g" % grad_factor,
                )
                logging.warning(
                    f"Gradient norm exceeds average of last {grad_norm_buffer_size} "
...
def _to_device(device, *args):
    ...
    if device == args[0].device:
        ...
        for src in args:
            numels = src.numel()
            dests.append(
                combined_dest[offset : offset + numels].reshape(src.shape)
            )
            offset += numels
        return tuple(dests)
...
def _get_target_rms(x: Tensor, min_target_rms: float) -> Tensor:
    ...
    if (
        x.ndim > 1 and __name__ == "__main__"
    ):  # will only be used for x.ndim > 1.
...
def _multiply_product_combined(
    l2: Tensor, grad: Tensor, dim: int, need_grad_sumsq: bool
):
    """
...
def _compute_l2_grad(
    ...
    for dim in range(x.ndim):
        ...
        if x.shape[dim] <= 1:
            num_ignored_dims += 1
            continue
        (l2, maybe_grad_sumsq) = _multiply_product_combined(
            l2, grad, dim, grad_sumsq is None
        )
    ...
    if safe and rho > 0:
        ...
        if random.random() < 0.001 and alpha < 1.0:
            logging.info(
                f"madam optimizer: alpha={alpha}, shape={tuple(x.shape)}"
            )
    return l2
...
class TestModel(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = torch.nn.Conv1d(
            in_channels=300, out_channels=200, kernel_size=1
        )
        self.relu = torch.nn.ReLU()
        self.conv2 = torch.nn.Conv1d(
            in_channels=200, out_channels=250, kernel_size=3
        )
...
def test_moam():
    ...
    # and we can more easily tell whether our update has the desired effect.
    optimizer = Moam(
        model.parameters(), factor=1.0, warm_step=300, min_target_rms=0.01
    )
