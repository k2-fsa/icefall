    ...
    params = AttributeDict(
        {
            ...
            "log_interval": 50,
            "reset_interval": 200,
            "valid_interval": 700,  # For the 100h subset, use 800
...
def compute_loss(
    ...
    #numel = params.world_size / (params.accum_grads * info["utterances"])
    info["loss"] = loss.detach().cpu().item()#*numel
    info["simple_loss"] = simple_loss.detach().cpu().item()#*numel
    info["pruned_loss"] = pruned_loss.detach().cpu().item()#*numel
...
def compute_wer(
    ...
    """Compute WER (%) over valid (dev-clean + dev-other) set."""
    model.eval()
    
    tot_wer = MetricsTracker()
    tot_wer["ref_words_num"] = 0
    tot_wer["subs_error_num"] = 0
    tot_wer["ins_error_num"] = 0
    tot_wer["del_error_num"] = 0
        ...
        if batch_idx % params.log_interval == 0:
            ...
            if tb_writer is not None:
                if params.multi_optim:
                    ...
                    tb_writer.add_scalar(
                        "train/dec_learning_rate", cur_enc_lr, params.batch_idx_train
                    )
...
def run(rank, world_size, args, wb=None):
    ...
    if params.multi_optim:
        ...
        """ 100h : 1600s in 1 batch -> 700 updates"""
        """ 960h : 1600s in 1 batch -> 6600 updates"""
        '''
        batch_durations = params.world_size * params.max_duration * params.accum_grads
        ...
        scheduler_enc = TriStageLRScheduler(
                                    optimizer_enc,
                                    init_lr=1e-05,
        ...
        scheduler_dec = TriStageLRScheduler(
                                    optimizer_dec,
                                    init_lr=1e-05,
                                    ...
                                    hold_steps=int(total_steps*0.49),
                                    decay_steps=int(total_steps*0.5),
                                    total_steps=total_steps,
                                    verbose=False,
                             )
...
def main():
    ...
    if args.wandb: 
        wb = wandb.init(project="d2v-T", entity="dohe0342", config=vars(args))
    else: 
        wb = None
