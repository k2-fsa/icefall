...
def _get_feat_extract_output_lengths(
    params, input_lengths: Union[torch.LongTensor, int]
):
    def _conv_out_length(input_length, kernel_size, stride):
        return (input_length - kernel_size) // stride + 1

    for kernel_size, stride in zip(
        _to_int_tuple(params.conv_kernel),
        _to_int_tuple(params.conv_stride),
    ):
...
def compute_loss(
    ...
    with warnings.catch_warnings():
        ...
        info["frames"] = (
            _get_feat_extract_output_lengths(params, audio_lens).sum().item()
        )
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        try:
            ...
            if params.batch_idx_train % params.accum_grad == 0:
...
def run(rank, world_size, args):
    ...
    def remove_short_and_long_utt(c: Cut):
        ...
        # In HuBERT, the conv module uses the following expression
        # for subsampling
        T = _get_feat_extract_output_lengths(params, c.num_samples)
