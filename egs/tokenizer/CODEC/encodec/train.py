...
def run(rank, world_size, args):
    ...
    codec_dataset = CodecDataModule(args)

    train_cuts = codec_dataset.train_all_shuf_cuts()
    ...
    train_dl = codec_dataset.train_dataloaders(
        train_cuts,
        world_size=world_size,
        rank=rank,
    )

    valid_cuts = codec_dataset.dev_cuts()
    valid_dl = codec_dataset.valid_dataloaders(
        valid_cuts,
        world_size=world_size,
        rank=rank,
    )
