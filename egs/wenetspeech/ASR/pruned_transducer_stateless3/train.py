...
from local.char_graph_compiler_with_abbreviations import (
...
def get_parser():
    ...
    parser.add_argument(
        "--lang-dir",
        type=str,
        default="data/lang_char_abbreviations",
        help="""The lang dir
...
def run(rank, world_size, args):
    ...
    if (
        ...
        and params.start_epoch == 0
        and params.training_subset == "L"
    ):
        logging.info("Loading pretrained model")
        checkpoint = torch.load(
            "pruned_transducer_stateless3/pretrained_epoch_10_avg_2.pt"
        )
        dst_state_dict = model.state_dict()
        src_state_dict = checkpoint["model"]
        if next(iter(src_state_dict)).startswith("module."):
            src_state_dict = src_state_dict.module.state_dict()
        pretrained_dict = {
            k: v
            for k, v in src_state_dict.items()
            if k in dst_state_dict.keys()
            and v.size() == dst_state_dict[k].size()
        }
        missed_params = [
            k
            for k, v in dst_state_dict.items()
            if k not in src_state_dict.keys()
        ]
        logging.info(
            "Loaded params/tot params: {}/{}".format(
                len(pretrained_dict), len(dst_state_dict)
            )
        )
        logging.info("Miss matched params: {}".format(missed_params))
