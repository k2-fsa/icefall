...
def save_results(
    ...
    for subset, prefix in subsets.items():
        ...
        for key, results in results_dict.items():
            recog_path = (
                params.res_dir
                / f"recogs-{test_set_name}-{subset}-{key}-{params.suffix}.txt"
            )
            ...
            errs_filename = (
                params.res_dir
                / f"errs-{test_set_name}-{subset}-{key}-{params.suffix}.txt"
            )
        ...
        errs_info = (
            params.res_dir
            / f"wer-summary-{test_set_name}-{subset}-{key}-{params.suffix}.txt"
        )
...
def main():
    ...
    test_eval2000_cuts = (
        switchboard.test_eval2000_cuts()
        .trim_to_supervisions(keep_all_channels=True)
        .filter(remove_short_utt)
    )
