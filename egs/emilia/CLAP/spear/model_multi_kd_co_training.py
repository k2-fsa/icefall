...
class MultiKDModel(nn.Module):
    ...
    def forward(
        ...
        Args:
          ...
          x_lens:
            ...
            before padding.
          codebook_indexes:
            Codebook indexes of teacher embeddings (N,T,C)
          at_targets:
            The audio tagging target (N, num_events)
        ...
        return codebook_loss, at_loss, co_training_loss
    ...
    def forward_co_training_loss(
        ...
        co_training_loss = nn.functional.kl_div(
            input=logits,
            target=exchanged_targets,
            reduction="none",
            log_target=True, # we do the log_softmax
        )  # (2 * N, T, C)
        length_mask = make_pad_mask(encoder_out_lens).unsqueeze(-1).unsqueeze(-1)
        
        N = encoder_out_lens.shape[0]
        num_cb = co_training_loss.shape[1]
        co_training_loss = co_training_loss.reshape(N, -1, *co_training_loss.shape[-2:])
        co_training_loss = co_training_loss.masked_fill(length_mask, 0.0)
        co_training_loss = co_training_loss.sum(dim=(1,2,3))/ num_cb
