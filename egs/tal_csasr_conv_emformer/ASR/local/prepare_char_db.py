...
def generate_lexicon(
    ...
    for word in words:
        ...
        if match:
            ...
            tokens = bpemb_en.encode(word.strip(" \t"))
...
def loadLangChar(filename):
  token_sym_table={}
  with open(filename,'r',encoding='utf-8') as fr:
    for line in fr:
      char,index = line.strip().split(' ')
      if char not in token_sym_table:
        token_sym_table[char]=int(index)
  return token_sym_table

def main():
    lang_dir = Path("data/lang_char")
    text_file = lang_dir / "text_with_bpe"
    bpe_model = lang_dir / "spm_model_name.model" 
    

    langChar = 'data/lang_char/tokens.txt'
    ...
    #token_sym_table = loadLangChar(langChar)
