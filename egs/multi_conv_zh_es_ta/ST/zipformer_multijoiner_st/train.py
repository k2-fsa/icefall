...
"""
This is implementation for multitask ASR ST model.
...
./zipformer_hat_st/train.py \
  ...
  --start-epoch 1 \
  --use-fp16 1 \
  --exp-dir zipformer_hat_st/exp-multi-joiner-nohat-pbe4k_crctc\
  --causal 0 \
  --num-encoder-layers 2,2,2,2,2,2,2,2 \
  --feedforward-dim 512,768,1024,1024,1024,1024,1024,768 \
  --encoder-dim 192,256,384,512,384,384,384,256 \
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    # # Config for ST encoder
    # parser.add_argument(
    #     "--use-st-encoder",
    #     type=str2bool,
    #     default=False,
    #     help="Whether to use an st encoder.",
    # )
    parser.add_argument(
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Select language for training [es,ar,zh]",
    )
...
def get_st_decoder_model(params: AttributeDict) -> nn.Module:
    st_decoder = Decoder(
        vocab_size=params.st_vocab_size,
        decoder_dim=params.decoder_dim,
...
def get_model(params: AttributeDict) -> nn.Module:
    ...
    model = StModel(
        ...
        decoder_dim=params.decoder_dim,
        vocab_size=params.vocab_size,
        st_vocab_size=params.st_vocab_size,
    ...
    with torch.set_grad_enabled(is_training):
        ...
        if not torch.all(is_finite):
            logging.info(
                ...
                f"pruned_loss_lm: {st_simple_loss}\n"
                f"pruned_loss_lm: {st_pruned_loss}\n"
            )
...
def run(rank, world_size, args):
    ...
    params.st_vocab_size = st_sp.get_piece_size()
    ...
    if params.lang:
        train_cuts = train_cuts.filter(lambda c: c.supervisions[0].language[0]==params.lang)
    ...
    if params.lang:
        valid_cuts = valid_cuts.filter(lambda c: c.supervisions[0].language[0]==params.lang)
