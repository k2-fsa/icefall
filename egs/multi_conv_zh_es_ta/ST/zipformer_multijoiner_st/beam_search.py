...
def greedy_search_st(
    ...
    encoder_out_st = model.st_joiner.encoder_proj(encoder_out)    
...
def modified_beam_search(
    ...
    encoder_out = model.joiner.encoder_proj(packed_encoder_out.data)
    encoder_out_st = model.st_joiner.encoder_proj(packed_encoder_out.data)
    ...
    for (t, batch_size) in enumerate(batch_size_list):
        ...
        decoder_input_st = torch.tensor(
            [hyp.ys[-context_size:] for hyps in A_st for hyp in hyps],
            device=device,
            dtype=torch.int64,
        )  # (num_hyps, context_size)
    ...
    unsorted_indices = packed_encoder_out.unsorted_indices.tolist()
    for i in range(N):
        ans_st.append(sorted_ans_st[unsorted_indices[i]])
        ans_timestamps_st.append(sorted_timestamps_st[unsorted_indices[i]])
