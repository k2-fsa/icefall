#!/usr/bin/env python3
# Copyright           2023  Johns Hopkins University (author: Amir Hussein)
...
"""
This model uses a hierarchical ASR-ST architecture with self-distillation (HENT_SRT) https://arxiv.org/abs/2506.02157.
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="If True, the HAT loss will be used",
    )
    parser.add_argument(
        "--use-lstm-predictor",
        type=str2bool,
        default=False,
        help="If True, LSTM predictor in translation",
    )
    ...
    parser.add_argument(
        "--st-num-encoder-layers",
        type=str,
        default="2,2,2,2",
        help="Number of zipformer encoder layers per stack, comma separated.",
    )
    parser.add_argument(
        "--st-downsampling-factor",
        type=str,
        default="2,4,8,4",
        help="Downsampling factor for each stack of encoder layers.",
    )
    ...
    parser.add_argument(
        "--st-encoder-unmasked-dim",
        type=str,
        default="192,192,256,192",
        help="Unmasked dimensions in the encoders, relates to augmentation during training.  "
        "A single int or comma-separated list.  Must be <= each corresponding encoder_dim.",
    )

    parser.add_argument(
        "--st-cnn-module-kernel",
        type=str,
        default="31,15,15,31",
        help="Sizes of convolutional kernels in convolution modules in each encoder stack: "
        "a single int or comma-separated list.",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="Scale for speech translation loss.",
    )
    parser.add_argument(
        "--st-joiner-dim",
        type=int,
...
def get_parser():
    ...
    parser.add_argument(
        "--model-dir",
        type=str,
        default="zipformer_hat/exp/pretrained.pt",
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="""If true, the main model is frozen. Only the auxiliary lid encoder
        is trained.
        """,
    )
    parser.add_argument(
...
def get_st_encoder_model(params: AttributeDict) -> nn.Module:
    st_encoder = Zipformer2(
        output_downsampling_factor=params.st_output_downsampling_factor,
        downsampling_factor=_to_int_tuple(params.st_downsampling_factor),
        num_encoder_layers=_to_int_tuple(params.st_num_encoder_layers),
        encoder_dim=_to_int_tuple(params.st_encoder_dim),
        encoder_unmasked_dim=_to_int_tuple(params.st_encoder_unmasked_dim),
        query_head_dim=_to_int_tuple(params.st_query_head_dim),
        pos_head_dim=_to_int_tuple(params.st_pos_head_dim),
        value_head_dim=_to_int_tuple(params.st_value_head_dim),
        pos_dim=params.st_pos_dim,
        num_heads=_to_int_tuple(params.st_num_heads),
        feedforward_dim=_to_int_tuple(params.st_feedforward_dim),
        cnn_module_kernel=_to_int_tuple(params.st_cnn_module_kernel),
        dropout=ScheduledFloat((0.0, 0.3), (20000.0, 0.1)),
        warmup_batches=4000.0,
        causal=params.causal,
        chunk_size=_to_int_tuple(params.st_chunk_size),
        left_context_frames=_to_int_tuple(params.st_left_context_frames),
    )
    return st_encoder
...
def get_lstm_decoder_model(params: AttributeDict) -> nn.Module:
    decoder = LSTMDecoder(
        vocab_size=params.vocab_size,
        blank_id=params.blank_id,
        decoder_dim=params.decoder_embedding_dim,
...
def get_st_decoder_model(params: AttributeDict) -> nn.Module:
    decoder = Decoder(
        vocab_size=params.vocab_st_size,
        decoder_dim=params.decoder_dim,
        blank_id=params.blank_st_id,
        context_size=params.st_context_size,
    )
    return decoder
...
def get_model(params: AttributeDict) -> nn.Module:
    ...
    if params.use_transducer:
        ...
        if params.use_st_joiner:
            st_joiner = get_st_joiner_model(params)
            if params.use_lstm_predictor:
                st_decoder = get_lstm_decoder_model(params)
            else:
                st_decoder = get_st_decoder_model(params)
            st_encoder = get_st_encoder_model(params)
        else:
            st_joiner = None
            st_decoder = None
            st_encoder = None
...
def load_checkpoint_if_available(
    ...
    if params.start_batch > 0:
        filename = Path(params.model_dir) / f"checkpoint-{params.start_batch}.pt"
    elif params.start_epoch > 1:
        filename = Path(params.model_dir) / f"epoch-{params.start_epoch}.pt"
    else:
        return None
...
def find_bad_length(feats, texts):
    ...
    # Print indices where the error occurs
    error_indices = errors.nonzero(as_tuple=True)[0].tolist()
    print(f"Indices where text length is greater than duration: {error_indices}")
    if error_indices:
...
def compute_loss(
    ...
    st_sp: spm.SentencePieceProcessor,
    batch: dict,
    is_training: bool,
    ...
    with torch.set_grad_enabled(is_training):
        ...
        simple_loss, st_simple_loss, pruned_loss, st_pruned_loss, ctc_loss, st_ctc_loss, cr_loss, st_cr_loss = model(
            ...
            st_prune_range=params.st_prune_range,
            am_scale=params.am_scale,
            lm_scale=params.lm_scale,
            use_asr_cr_ctc=use_asr_cr_ctc,
            use_st_cr_ctc=use_st_cr_ctc,
        ...
        if params.use_st_joiner:
            ...
            is_finite = (
                ...
                & st_simple_loss_is_finite
                & st_pruned_loss_is_finite
            )
            if not torch.all(is_finite):
                logging.info(
                    ...
                    f"simple_losses_st: {st_simple_loss}\n"
                    f"pruned_loss_st: {st_pruned_loss}\n"
                )
    ...
    if params.use_st_ctc:
        info["st_ctc_loss"] = st_ctc_loss.detach().cpu().item()
        if params.use_st_cr_ctc:
            info["cr_st_loss"] = st_cr_loss.detach().cpu().item()
    return loss, info
...
def run(rank, world_size, args):
    ...
    st_sp.load(params.bpe_st_model)

    # <blk> is defined in local/train_bpe_model.py
    params.blank_id = sp.piece_to_id("<blk>")
    params.blank_st_id = st_sp.piece_to_id("<blk>")
    params.vocab_size = sp.get_piece_size()
    params.vocab_st_size = st_sp.get_piece_size()
    ...
    if params.use_asr_cr_ctc or params.use_st_cr_ctc:
        assert params.use_ctc or params.use_st_ctc
    ...
    multling = MultiLingAsrDataModule(args)

    train_cuts = multling.train_cuts()
