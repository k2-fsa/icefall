...
(1) greedy search
# Streaming
    ./zipformer_hat_st_hier_cr_ctc/decode_st.py \
        --epoch 20 --avg 13 --use-averaged-model 0 \
        --causal 1 \
        --exp-dir zipformer_hat_st_hier_cr_ctc/exp-st_causal \
...
./zipformer_hat_st_hier_cr_ctc/decode_st.py \
    --epoch 20 --avg 13 --use-averaged-model 0 \
    --causal 0 \
    --exp-dir zipformer_hat_st_hier_cr_ctc/exp-st \
    --bpe-model data/lang_bpe_5000/bpe.model \
    --bpe-st-model data/lang_st_bpe_4000/bpe.model \
    --output-downsampling-factor 2 \
    --st-output-downsampling-factor 1 \
...
def decode_one_batch(
    ...
    if params.decoding_method == "greedy_search" and params.max_sym_per_frame == 1:
        hyp_tokens = greedy_search_batch_st(
            model=model,
        ...
        hyp_tokens = modified_beam_search_st2(
            model=model,
            encoder_out=encoder_out,
            encoder_out_lens=encoder_out_lens,
            st_encoder_out=st_encoder_out,
       ...
       for hyp in sp.decode(hyp_tokens):
        ...
        for i in range(batch_size):
            ...
            encoder_out_st_i = st_encoder_out[i:i+1, :st_encoder_out_lens[i]]
            # fmt: on
...
def main():
    ...
    if params.st_blank_penalty:
        params.suffix += f"-blank-penalty-{params.st_blank_penalty}"
    ...
    params.vocab_st_size = sp_st.get_piece_size()
    ...
    # test_ta1_dl = multiling.test_dataloaders(test_ta1)
    # dev_ta_dl = multiling.test_dataloaders(dev_ta)
    test_bnn_dl = multiling.test_dataloaders(test_bnn)
    ...
    # test_dl = [test_fisher_dl, test_iwslt22_dl, test_bnn_dl, test_callhome_dl, test_fisher_cs_dl]
    test_dl = [test_fisher_dl, test_iwslt22_dl, test_bnn_dl]
    # test_dl = [dev_ta_dl, dev_sp_dl, dev_zh_dl]
    # test_dl = [test_iwslt22_dl, test_bnn_dl]
    # test_sets = ["test-fisher-cs"]
    # test_dl = [test_fisher_cs_dl]
