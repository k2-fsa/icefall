...
"""
This model uses the LID bank for ASR
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    # Config for lidiliary LID encoder
    parser.add_argument(
        "--lid-output-layer",
        type=int,
        default=-1,
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="Whether to use an lid encoder.",
    )
    parser.add_argument(
        "--lid-num-encoder-layers",
        type=str,
        default="2,2,2",
        help="Number of zipformer encoder layers per stack, comma separated.",
    )
    parser.add_argument(
        "--lid-downsampling-factor",
        type=str,
        default="2,4,2",
        help="Downsampling factor for each stack of encoder layers.",
    )
    ...
    parser.add_argument(
        "--lid-pos-emb-skip-rate-schedule",
        type=str,
        default="none",
...
def get_lid_encoder_model(params: AttributeDict) -> nn.Module:
    lid_encoder = Zipformer2(
        output_downsampling_factor=2,
        downsampling_factor=_to_int_tuple(params.lid_downsampling_factor),
        num_encoder_layers=_to_int_tuple(params.lid_num_encoder_layers),
        encoder_dim=_to_int_tuple(params.lid_encoder_dim),
        encoder_unmasked_dim=_to_int_tuple(params.lid_encoder_unmasked_dim),
        query_head_dim=_to_int_tuple(params.lid_query_head_dim),
        pos_head_dim=_to_int_tuple(params.lid_pos_head_dim),
        value_head_dim=_to_int_tuple(params.lid_value_head_dim),
        pos_dim=params.lid_pos_dim,
        num_heads=_to_int_tuple(params.lid_num_heads),
        feedforward_dim=_to_int_tuple(params.lid_feedforward_dim),
        cnn_module_kernel=_to_int_tuple(params.lid_cnn_module_kernel),
        dropout=ScheduledFloat((0.0, 0.3), (20000.0, 0.1)),
        warmup_batches=4000.0,
        causal=params.causal,
        chunk_size=_to_int_tuple(params.lid_chunk_size),
        left_context_frames=_to_int_tuple(params.lid_left_context_frames),
    )
    return lid_encoder
...
def get_joiner_model(params: AttributeDict) -> nn.Module:
    joiner = Joiner(
        ...
        vocab_size=params.vocab_size,
        encoder_lid=int(params.lid_encoder_dim.split(",")[-1]),
    )
    return joiner
...
def get_model(params: AttributeDict) -> nn.Module:
    ...
    if params.use_transducer:
        ...
        if params.use_lid_joiner:
            lid_joiner = get_lid_joiner_model(params)
            lid_encoder = get_lid_encoder_model(params)
        else:
            lid_joiner = None
            lid_encoder = None
    else:
    ...
    model = AsrModel(
        ...
        decoder=decoder,
        joiner=joiner,
        lid_joiner=lid_joiner,
        lid_encoder=lid_encoder,
        encoder_dim=int(params.encoder_dim.split(",")[-1]),
        decoder_dim=params.decoder_dim,
        vocab_size=params.vocab_size,
...
def load_checkpoint_if_available(
    ...
    if params.start_batch > 0:
        ...
        filename = Path(params.model_dir) / f"epoch-{params.start_epoch-1}.pt"
    else:
        return None
...
def compute_loss(
    ...
    y_list = [sp.encode(t, out_type=int) for t in text_list]
    num_tokens = [[len(t) for t in utt] for utt in y_list]
    ...
    y_lid = k2.RaggedTensor(lids).to(device)

    y = sp.encode(texts, out_type=int)
    y = k2.RaggedTensor(y)
    with torch.set_grad_enabled(is_training):
        simple_loss, pruned_loss, ctc_loss, lid_pruned_loss = model(
            x=feature,
            x_lens=feature_lens,
            y=y,
            y_lid=y_lid,
...
def run(rank, world_size, args):
    ...
    train_dl = seame.train_dataloaders(
        train_cuts, sampler_state_dict=sampler_state_dict
    )

    valid_cuts = seame.valid_cuts()
    valid_dl = seame.valid_dataloaders(valid_cuts)
