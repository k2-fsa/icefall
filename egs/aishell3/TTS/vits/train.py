...
from tts_datamodule import Aishell3SpeechTtsDataModule
...
def prepare_input(
    ...
    tokens = batch["tokens"]
    speakers = (
        torch.Tensor([speaker_map.get(sid, 0) for sid in batch["speakers"]])
        .int()
        .to(device)
    )
...
def run(rank, world_size, args):
    ...
    train_cuts = aishell3.train_cuts()
    ...
    train_dl = aishell3.train_dataloaders(train_cuts)

    valid_cuts = aishell3.valid_cuts()
    valid_dl = aishell3.valid_dataloaders(valid_cuts)
...
def main():
    parser = get_parser()
    Aishell3SpeechTtsDataModule.add_arguments(parser)
