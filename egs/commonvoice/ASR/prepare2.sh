...
if [ $stage -le 4 ] && [ $stop_stage -ge 4 ]; then
  ...
  mkdir -p data/fbank
  if [ ! -e data/fbank/.commonvoice_dev_test.done ]; then
    ./local/compute_fbank_commonvoice_dev_test.py --language $lang
    touch data/fbank/.commonvoice_dev_test.done
  fi
fi
...
if [ $stage -le 5 ] && [ $stop_stage -ge 5 ]; then
  ...
  if [ ! -e $split_dir/.train_split.done ]; then
    lhotse split $num_splits ./data/fbank/cv-${lang}_cuts_train_raw.jsonl.gz $split_dir
    touch $split_dir/.train_split.done
  fi
fi

if [ $stage -le 6 ] && [ $stop_stage -ge 6 ]; then
  log "Stage 6: Compute features for train subset of CommonVoice"
  if [ ! -e data/fbank/.commonvoice_train.done ]; then
...
if [ $stage -le 8 ] && [ $stop_stage -ge 8 ]; then
  ...
  for vocab_size in ${vocab_sizes[@]}; do
    ...
    if [ ! -f $lang_dir/transcript_words.txt ]; then
      ...
      files=$(
        find "$dl_dir/CommonVoice/train-clean-100" -name "*.trans.txt"
        find "$dl_dir/CommonVoice/train-clean-360" -name "*.trans.txt"
        find "$dl_dir/CommonVoice/train-other-500" -name "*.trans.txt"
      )
      for f in ${files[@]}; do
        cat $f | cut -d " " -f 2-
