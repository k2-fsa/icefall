...
from asr_finetune_datamodule import MGB2AsrDataModule
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--model-dir",
        type=str,
        default="/alt-arabic/speech/amir/kanari_models/k2/stateless_tranducer5/pruned_transducer_stateless5/exp_streaming/best-valid-loss.pt",
    ...
    params = AttributeDict(
        {
            ...
            "log_interval": 50,
            "reset_interval": 200,
            "valid_interval": 1000,  # For the subset, use 3000
...
def load_checkpoint_if_available(
    ...
    elif params.model_dir:
        filename = Path(params.model_dir)
    else:
        return None
...
def compute_validation_loss(
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(valid_dl):
            ...
            if feature.size(0) == feature_lens.size(0):
...
def run(rank, world_size, args):
    ...
    # def remove_bad(c: Cut):
    #     tol = 1e-2
    #     if (c.supervisions[0].end > c.duration + tol) or (c.supervisions[0].start < -tol):
    ...
    # train_cuts = train_cuts.filter(remove_bad)
