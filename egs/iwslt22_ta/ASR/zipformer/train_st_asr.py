...
./zipformer/train_st.py \
  ...
  --start-epoch 1 \
  --use-fp16 1 \
  --exp-dir zipformer/exp-st-small \
  --causal 0 \
...
./zipformer/train_st.py \
  ...
  --start-epoch 1 \
  --use-fp16 1 \
  --exp-dir zipformer/exp-st-small2 \
  --causal 0 \
  --num-encoder-layers 2,2,2,2,2,2 \
...
from model import Transducer_asr_st
...
def get_transducer_model(params: AttributeDict) -> nn.Module:
    ...
    decoder_tgt = get_decoder_model(params)
    joiner_tgt = get_joiner_model(params)

    model = Transducer_asr_st(
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        simple_loss, pruned_loss, simple_loss_tgt, pruned_loss_tgt = model(
            x=feature,
            x_lens=feature_lens,
            y=y,
            y_tgt=y_tgt,
            prune_range=params.prune_range,
            prune_range_tgt=params.prune_range_tgt
            am_scale=params.am_scale,
            lm_scale=params.lm_scale)
        
        simple_loss_is_finite = torch.isfinite(simple_loss)
        pruned_loss_is_finite = torch.isfinite(pruned_loss)
        simple_loss_is_finite_tgt = torch.isfinite(simple_loss_tgt)
        pruned_loss_is_finite_tgt = torch.isfinite(pruned_loss_tgt)
        ...
        is_finite_tgt = simple_loss_is_finite_tgt & pruned_loss_is_finite_tgt
        inf_flag_tgt = False
        if not torch.all(is_finite_tgt):
            inf_flag_tgt = True
            logging.info(
                "Not all losses are finite!\n"
                f"simple_loss: {simple_loss_tgt}\n"
                f"pruned_loss: {pruned_loss_tgt}"
            )
            display_and_save_batch(batch, params=params, sp=sp, sp_tgt=sp_tgt)
            simple_loss_tgt = simple_loss_tgt[simple_loss_is_finite]
            pruned_loss_tgt = pruned_loss_tgt[pruned_loss_is_finite]

        simple_loss_tgt = simple_loss_tgt.sum()
        pruned_loss_tgt = pruned_loss_tgt.sum()
        ...
        loss_tgt = simple_loss_scale * simple_loss_tgt + pruned_loss_scale * pruned_loss_tgt
        loss_tot = param.alpha * loss + (1-param.alpha) * loss_tgt

    assert loss_tot.requires_grad == is_training

    info = MetricsTracker()
    info_tgt = MetricsTracker()
    ...
    info_tgt["loss_tgt"] = loss_tgt.detach().cpu().item()
    info_tgt["simple_loss_tgt"] = simple_loss_tgt.detach().cpu().item()
    info_tgt["pruned_loss_tgt"] = pruned_loss_tgt.detach().cpu().item()

    return loss_tot, info, info_tgt, inf_flag, inf_flag_tgt
...
def compute_validation_loss(
    ...
    for batch_idx, batch in enumerate(valid_dl):
        loss, loss_info, loss_info_tgt, inf_flag, inf_flag_tgt = compute_loss( 
            params=params,
            model=model,
            sp=sp,
            sp_tgt=sp_tgt,
...
def run(rank, world_size, args):
    ...
    params.blank_id_tgt = sp_tgt.piece_to_id("<blk>")
    params.vocab_size_tgt = sp_tgt.get_piece_size()
    ...
    def remove_short_and_long_utt(c: Cut):
        ...
        tokens = sp.encode(c.supervisions[0].custom['tgt_text'], out_type=str)
