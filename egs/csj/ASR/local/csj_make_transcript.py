...
transcripts in accordance to the tag decisions listed in the .ini file.
...
class CSJSDB_Word:
    ...
    def _parse_surface(self):
        ...
        # This is for pauses <P:00453.373-00454.013>
        self.surface = re.sub(
            r"<P:.+>", REPLACEMENTS_SURFACE["<P>"], self.surface
        )
        ...
        for t, decision in decisions.items():
            ...
            if isinstance(decision, int):
                ...
                if decision[:5] == "eval:":
                    ...
                    ret = PLUS.join(
                        decision for _ in range(choices[0].count(PLUS) + 1)
                    )
...
def modify_text(
    ...
    for word in word_list:
        ...
        if word.spk_id not in line_sgid:
            ...
            out.append(
                f"{line_sgid} {line_start:09.3f} {line_end:09.3f} " + line
            )
    ...
    for word in word_list:
        ...
        if last_sgid == word.sgid and last_spk_id == word.spk_id:
            ...
            if minlen < (last_end - line_start) and "×" not in line:
                ...
                segments.append(
                    (f"{line_spk_id}_{line_sgid}", line_start, last_end)
                )
...
def create_trans_dir(corpus_dir: Path, trans_dir: Path):
    ...
    for ori_files in (corpus_dir / "MORPH/SDB").glob("*/*.sdb"):
        ...
        if spk_id[0] == "D":
            ...
            (new_dir / f"{spk_id}-wav.list").write_text(
                wav.as_posix(), encoding="utf8"
            )
    ...
    def parse_one_sdb(sdb: Path):
        ...
        if not use_segments:
            ...
            for channel in channels:
                segments = (
                    Path(sdb.as_posix()[:-4] + channel).read_text().split("\n")
                )
...
def get_args():
    ...
    parser.add_argument("--corpus-dir", type=Path, help="Path to corpus")
    parser.add_argument(
        "--trans-dir", type=Path, help="Path to output transcripts"
    )
    parser.add_argument("--config", type=Path, help="Path to config")
    parser.add_argument(
        "-j", "--num-jobs", type=int, default=16, help="Number of jobs to start"
    )
    parser.add_argument(
        "--write-segments",
        action="store_true",
