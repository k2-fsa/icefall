...
class Transducer(nn.Module):
    ...
    def __init__(
        ...
        self.final_proj = nn.Linear(joiner_dim, encoder_dim)
    ...
    def forward_speech_encoder(
        ...
        encoder_out, x_lens, target_layer_results, mask_indices = self.encoder(x, x_lens)
        assert torch.all(x_lens > 0)
        ...
        if with_proj:
          ...
          encoder_out = encoder_out[mask_indices]
          return(self.final_proj(encoder_out), mask_indices)
        else:
          return(target_layer_results, mask_indices)
