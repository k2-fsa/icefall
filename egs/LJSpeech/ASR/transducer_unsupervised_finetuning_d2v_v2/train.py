...
def compute_loss(
    ...
    batch: dict,
    is_training: bool,
    ema_model: Union[nn.Module, DDP] = None,
    decode: bool = False,
    ...
    with torch.no_grad():
        ema_model.eval()
        ema_model.module.encoder.encoders.training = False

        average_top_k_layers = params.average_top_k_layers
        target_layer_results, _ = ema_model.module.forward_speech_encoder(
            x=feature,
            x_lens=feature_lens,
            with_proj=False,
            )
        target_layer_results = [l[2] for l in target_layer_results]
        target_layer_results[len(target_layer_results) - average_top_k_layers:]

    with torch.set_grad_enabled(is_training):
        instance_norm_target_layer = params.instance_norm_target_layer
        ...
        x, mask_indices = model.module.forward_speech_encoder(
            x=feature,
            x_lens=feature_lens,
            with_proj=True,
            )
        y = y[mask_indices] # FIXME: what about padding mask? It might be a critical problem
    ...
    # FIXME!
    # with torch.no_grad():
    #     info["target_var"] = compute_var(y)
    #     info["pred_var"] = compute_var(x.float())
    ...
    for epoch in range(params.start_epoch, params.num_epochs + 1):
        ...
        if params.update_ema:
            with torch.no_grad():
                ...
                stu_state_dict = {}
                for name, p in model.named_parameters():
                    stu_state_dict[name] = p
                
                for name, p in ema_model.named_parameters():
                    p_org = stu_state_dict[name]
