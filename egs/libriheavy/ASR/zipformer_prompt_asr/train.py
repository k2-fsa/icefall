...
from dataset import triplet_text_sampling, naive_triplet_text_sampling, random_shuffle_subset
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--num-text-encoder-layers",
        type=str,
        default="2,4,6,6",
        help="The number of layers in text encoder"
    )
    
    parser.add_argument(
        "--text-encoder-unmasked-dim",
        type=str,
    ...
    parser.add_argument(
        "--text-encoder-lr-scale",
        type=float,
        default=0.75,
        help="Scale the lr of the text encoder to stablize training."
    )
...
def get_text_embed(params: AttributeDict) -> nn.Module:
    return TextEmbedding(
        num_embeddings=256,
        layer2_channels=_to_int_tuple(params.text_encoder_dim)[0],
    )
...
def get_text_encoder(params: AttributeDict) -> nn.Module:
    return Zipformer2(
        ...
        num_encoder_layers=_to_int_tuple(params.num_text_encoder_layers),
        encoder_dim=_to_int_tuple(params.text_encoder_dim),
        encoder_unmasked_dim=_to_int_tuple(params.text_encoder_unmasked_dim),
        ...
        causal=False, # The text encoder does not have to be causal
    )
...
def get_transducer_model(params: AttributeDict) -> nn.Module:
    lr_scale = params.text_encoder_lr_scale
    encoder_embed = get_encoder_embed(params)
    encoder = get_encoder_model(params)
    text_embed = get_text_embed(params)
    text_embed.lr_scale = lr_scale
    logging.info(f"Text_embed: {text_embed}")
    logging.info(f"Text_embed lr_scale: {lr_scale} ")
    ...
    text_encoder.lr_scale = lr_scale
    logging.info(f"Text_encoder lr_scale: {lr_scale} ")
...
def compute_loss(
    ...
    # Shuffle the pre_text and style texts
    if is_training:
        pre_texts = random_shuffle_subset(pre_texts, p=0.05, p_mask=0.05) # randomly shuffle the pre_text
        
        if random.random() < 0.5 or True: 
            # randomly shuffle the style_text
            style_texts = random_shuffle_subset(style_texts, p=0.2, p_mask=0.05) # now the style_texts are all in gt format
    ...
    text, text_lens, style_lens = _encode_texts_as_bytes(
        pre_texts,
        style_texts,
        device,
        max_len=max(feature_lens)//2,
    )
