...
def greedy_search_with_context(
    model: Transducer, encoder_out: torch.Tensor, context: torch.Tensor, max_sym_per_frame: int
) -> List[int]:
    """Greedy search for a single utterance.
    Args:
      model:
    ...
    while t < T and sym_per_utt < max_sym_per_utt:
        ...
        # fmt: on
        logits = model.joiner(
            current_encoder_out, decoder_out.unsqueeze(1), context=context, project_input=False
        )
        # logits is (1, 1, 1, vocab_size)
...
def greedy_search_batch_with_context(
    model: Transducer,
    encoder_out: torch.Tensor,
    encoder_out_lens: torch.Tensor,
    ...
    for batch_size in batch_size_list:
        ...
        current_encoder_out = encoder_out.data[start:end] # already gone through encoder_proj
        current_encoder_out = current_encoder_out.unsqueeze(1).unsqueeze(1)
        # current_encoder_out's shape: (batch_size, 1, 1, encoder_out_dim)
        offset = end

        decoder_out = decoder_out[:batch_size]
        cur_context = context[sorted_indices[:batch_size]] # get the context embedding
